...
// Disable timeout limits for initialization
set_time_limit(0);
...
try {
    ...
    // 2. Check if tables are already seeded
    $stmt = $pdo->query("SELECT COUNT(*) FROM users");
    $userCount = $stmt->fetchColumn();

    if ($userCount > 0) {
        echo "Database is already seeded.<br>";
        sendResponse(["success" => true, "message" => "Database already initialized and seeded."]);
        exit();
    }
    ...
    $insertUser = $pdo->prepare("INSERT INTO users (id, name, email, password, phone, college, year, role, avatar, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    foreach ($students as $stu) {
        $insertUser->execute($stu);
    }

    // Seed Admin
    $insertUser->execute(['ADMIN', 'Admin User', 'admin@oc2.in', 'admin123', '0000000000', 'OCC Headquarters', 'N/A', 'admin', 'AD', '2025-01-01']);
    ...
    $insertCourse = $pdo->prepare("INSERT INTO courses (id, title, category, duration, level, price, rating, enrolled, published, icon, instructor, description, modules, image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    $insertEnrollment = $pdo->prepare("INSERT INTO enrollments (id, student_id, course_id, progress, start_date, completed_modules, certificate_issued) VALUES (?, ?, ?, ?, ?, ?, ?)");
    foreach ($enrollments as $e) {
        $insertEnrollment->execute($e);
    }
    echo "Enrollments seeded.<br>";

    // 6. Seed Classes
    $classes = [
        ['CLS001', 'CRS001', 'Aptitude Masterclass', '2025-07-05', '10:00 AM', 'Vinay Prem Upadhyay', 'https://meet.google.com/abc-defg-hij', json_encode(['STU001', 'STU004', 'STU007', 'STU010'])],
        ['CLS002', 'CRS002', 'Deep Learning Workshop', '2025-07-06', '2:00 PM', 'King Upadhyay', 'https://meet.google.com/klm-nopq-rst', json_encode(['STU002', 'STU005', 'STU007'])],
        ['CLS003', 'CRS003', 'React.js Live Coding', '2025-07-07', '11:00 AM', 'Akash Dash', 'https://meet.google.com/uvw-xyza-bcd', json_encode(['STU001', 'STU003', 'STU008'])],
        ['CLS004', 'CRS004', 'System Design Basics', '2025-07-08', '4:00 PM', 'Monika Upadhyay', 'https://meet.google.com/efg-hijk-lmn', json_encode(['STU002', 'STU006', 'STU010'])],
        ['CLS005', 'CRS005', 'ATS Resume Workshop', '2025-07-09', '10:00 AM', 'Monika Upadhyay', 'https://meet.google.com/opq-rstu-vwx', json_encode(['STU004', 'STU008'])]
    ];

    $insertClass = $pdo->prepare("INSERT INTO classes (id, course_id, title, date, time, instructor, meeting_link, attendees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    foreach ($classes as $cl) {
        $insertClass->execute($cl);
    }
    echo "Classes seeded.<br>";

    // 7. Seed Assignments
    $assignments = [
        ['ASG001', 'CRS001', 'Mock Interview Recording', '2025-07-15', 'Record a 10-minute mock interview and upload the video link.'],
        ['ASG002', 'CRS002', 'Build a CNN Classifier', '2025-07-20', 'Build a CNN model to classify CIFAR-10 images with >85% accuracy.'],
        ['ASG003', 'CRS003', 'Full Stack Todo App', '2025-07-25', 'Build a complete Todo app with React frontend and Node.js backend with MongoDB.'],
        ['ASG004', 'CRS004', 'System Design Document', '2025-07-30', 'Design a scalable URL shortener system. Include architecture diagram, database schema, and API design.']
    ];

    $insertAsg = $pdo->prepare("INSERT INTO assignments (id, course_id, title, due_date, description) VALUES (?, ?, ?, ?, ?)");
    foreach ($assignments as $asg) {
        $insertAsg->execute($asg);
    }
    echo "Assignments seeded.<br>";

    // Seed Submissions (for ASG001 and ASG002)
    $submissions = [
        ['SUB001', 'ASG001', 'STU001', '2025-07-10', 'mock_interview_rahul.mp4', 85],
        ['SUB002', 'ASG001', 'STU004', '2025-07-12', 'mock_interview_sneha.mp4', 92],
        ['SUB003', 'ASG002', 'STU002', '2025-07-18', 'cnn_classifier_priya.ipynb', 95]
    ];

    $insertSub = $pdo->prepare("INSERT INTO submissions (id, assignment_id, student_id, submitted_at, file_name, grade) VALUES (?, ?, ?, ?, ?, ?)");
    foreach ($submissions as $sub) {
        $insertSub->execute($sub);
    }
    echo "Submissions seeded.<br>";
    ...
    $insertJob = $pdo->prepare("INSERT INTO jobs (id, company, role, type, domain, location, stipend, skills, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    foreach ($jobs as $j) {
        $insertJob->execute($j);
    }
    echo "Jobs seeded.<br>";
    ...
    $insertPayment = $pdo->prepare("INSERT INTO payments (id, student_id, course_id, amount, method, date, status, receipt_no) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    foreach ($payments as $p) {
        $insertPayment->execute($p);
    }
    echo "Payments seeded.<br>";
    ...
    $insertContact = $pdo->prepare("INSERT INTO contacts (id, name, email, phone, subject, message, date, read) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    foreach ($contacts as $c) {
        $insertContact->execute($c);
    }
    echo "Contacts seeded.<br>";

    // Commit Transaction
    $pdo->commit();
    echo "Database successfully initialized and seeded.<br>";
    sendResponse(["success" => true, "message" => "Database successfully initialized and seeded."]);
